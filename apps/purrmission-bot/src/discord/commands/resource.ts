...
/**
 * Register a new protected resource and set the caller as the owner.
 */
...
async function handleFieldsAdd(
  ...
  const isValidName = name.length > 0 && name.length <= 64 && /^[A-Za-z0-9_-]+$/.test(name);
...
async function handleFieldsGet(
  ...
  try {
    ...
    await interaction.reply({
      content: "⚠️ I couldn't send you a DM. Please enable DMs from this server and try again.",
      ephemeral: true,
    });
  }
}
