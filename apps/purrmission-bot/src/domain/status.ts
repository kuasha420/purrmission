...
import { EmbedBuilder, Client, TextChannel, Colors, time, TimestampStyles } from 'discord.js';
  ...
  async sendOnlineAnnouncement(client: Client): Promise<void> {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `The bot is now online and protecting your resources! Built with 💖 by the PurrfectSoft Team.`
        )
        ...
        .addFields(
          ...
          },
          {
            ...
            value: [
              '• `/resource list` - View protected resources',
              '• `/2fa get` - Get your 2FA codes',
              '• `/guardian add` - Add a resource guardian',
              '• `/access approve` - Approve a pending request (buttons preferred!)',
            ].join('\n'),
          },
          {
            name: '🛡️ Need Help?',
