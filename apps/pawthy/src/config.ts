...
function readLocalConfig(): LocalConfig | null {
  ...
  try {
    ...
    if (parsed.token !== undefined && typeof parsed.token !== 'string') {
      console.log(
        chalk.yellow(
          `Warning: Invalid token type in local config at ${configPath}. Expected string.`
        )
      );
      return null;
    }
    ...
    console.log(
      chalk.yellow(
        `Warning: Could not read local config at ${configPath}. It may be malformed or have permission issues.`
      )
    );
    return null;
  }
...
function writeLocalConfig(localConfig: LocalConfig): void {
  ...
  try {
    ...
    console.error(
      chalk.red(
        `Error: Could not write to local config file at ${configPath}. Please check permissions.`
      )
    );
    process.exit(1);
  }
...
export function ensureGitignore(): void {
  ...
  try {
    if (fs.existsSync(gitignorePath)) {
      ...
      console.log(
        chalk.yellow(
          `Warning: No .gitignore found. Consider adding '${pawthyEntry}' to your .gitignore`
        )
      );
    }
  } catch (error) {
